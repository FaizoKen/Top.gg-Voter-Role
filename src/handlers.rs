...
// this plugin as a link plugin (a "Vote on Top.gg" Link button). Its editor
// probes this endpoint to show a live "Ready / Needs setup" state for the
// connected server. Public and CORS-open on purpose: it returns only whether
// the guild has any registration — never configuration content.

const PLUGIN_SLUG: &str = "topgg-voter-role";
...
pub struct DweebStatusQuery {
    #[serde(default)]
    pub guild: Option<String>,
}

/// JSON response with the CORS + cache headers the probe needs.
fn probe_json(
    status: StatusCode,
...
pub async fn dweeb_status(
    State(state): State<AppState>,
    axum::extract::Query(q): axum::extract::Query<DweebStatusQuery>,
) -> axum::response::Response {
    let Some(guild) = q.guild.as_deref().filter(|g| is_snowflake(g)) else {
        return probe_json(
            StatusCode::BAD_REQUEST,
            "no-store",
            serde_json::json!({ "error": "guild must be a Discord server id" }),
        );
    };
    ...
    let role_count: i64 =
            ...
            .await
        {
            Ok(n) => n,
            Err(e) => {
                error!("dweeb status probe query failed: {e}");
                return probe_json(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "no-store",
